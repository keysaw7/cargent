...
import { BenchmarkDetailList } from "@/components/cards/benchmark-detail-list";
...
import { modelCategoryLabel } from "@/lib/model-benchmarks";
...
export default async function CardPage({ params }: { params: Params }) {
  ...
  return (
    <main className="mx-auto grid w-full max-w-6xl gap-10 px-4 py-12 lg:grid-cols-[360px_minmax(0,1fr)]">
      <TradingCard card={view} size="lg" footerVisual="radar" className="mx-auto" />
      <div>
        ...
        {card.kind === "model" ? (
          <div className="mt-8 space-y-8">
            {category ? (
              <BenchmarkDetailList category={category} benchmarks={view.benchmarks ?? []} />
            ) : (
